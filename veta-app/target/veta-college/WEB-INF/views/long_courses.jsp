...
<div class="page-hero">
  <div class="wrap">
    <div class="breadcrumb"><a href="${pageContext.request.contextPath}/home"><%= sw?"Nyumbani":"Home" %></a><span class="bcs">›</span><span><%= sw?"Kozi Ndefu":"Long Courses" %></span></div>
    <h1><%= sw?"Kozi Ndefu — Zilizoidhinishwa na NVA":"Long Courses — NVA Certified" %></h1>
    <p><%= sw?"Programu za miaka 2–3 zinazosababisha Vyeti NVA 2–3.":"2–3 year programs leading to NVA Level 2–3 Certificates." %></p>
  </div>
</div>
      ...
      <c:when test="${not empty courses}">
        ...
        <div class="prog-card">
          <div class="prog-head" style="background:linear-gradient(135deg,#0A2463,#1565C0)">
            ...
            <div class="prog-type">${c.ntaLevel} · <%= sw?"KOZI NDEFU":"LONG COURSE" %></div>
            <h3>${c.nameEn}</h3>
...
<div class="prog-row">
    ...
    <span class="prv">
        ${c.getDurationDisplay()} (Cert) / ${c.getDurationMonths() / 12 + 1} yrs (Dip)
            ...
            <div class="prog-row"><span class="prl"><%= sw?"Ada/Mwaka":"Annual Fee" %></span><span class="prv">TZS <fmt:formatNumber value="${c.annualFee}" pattern="#,##0"/></span></div>
            <div class="prog-row"><span class="prl"><%= sw?"Usajili (mara moja)":"Registration (once)" %></span><span class="prv">TZS <fmt:formatNumber value="${c.registrationFee}" pattern="#,##0"/></span></div>
            <div class="prog-row"><span class="prl"><%= sw?"Ada ya Mtihani/mwaka":"Exam Fee/yr" %></span><span class="prv">TZS <fmt:formatNumber value="${c.examFee}" pattern="#,##0"/></span></div>
            ...
            <div style="margin-top:12px;background:var(--off);border-radius:8px;padding:11px;font-size:.8rem;color:var(--g700)">
              <strong style="display:block;margin-bottom:5px;color:var(--navy)"><%= sw?"Mahitaji ya Kuingia":"Entry Requirements" %></strong>
