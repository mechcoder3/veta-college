...
<div class="page-hero">
  <div class="wrap" style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px">
    <div>
      <h1 style="color:#fff;margin-bottom:4px"><%= sw?"Dashibodi ya Mwanafunzi":"Student Dashboard" %></h1>
      <p><%= sw?"Karibu tena,":"Welcome back," %> <c:out value="${sessionScope.userFullName}"/></p>
    </div>
    <form method="post" action="${pageContext.request.contextPath}/portal">
      <input type="hidden" name="action" value="logout">
      <button type="submit" class="btn btn-outline-w btn-sm">🚪 <%= sw?"Toka":"Logout" %></button>
    </form>
  </div>
</div>
...
<div style="padding:36px 0;background:var(--off)"><div class="wrap">
  ...
  <div style="background:linear-gradient(135deg,var(--navy),var(--blue));border-radius:var(--radius);padding:24px 28px;color:#fff;margin-bottom:24px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:14px">
    <div>
      <div style="font-size:.78rem;color:rgba(255,255,255,.6);margin-bottom:3px"><%= sw?"Karibu tena,":"Welcome back," %></div>
  ...
  <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:24px">
    <div class="pdsc"><div class="psi">📋</div><div class="psv" style="color:var(--green)">Active</div><div class="psl"><%= sw?"Hali":"Status" %></div></div>
    <div class="pdsc"><div class="psi">💳</div><div class="psv" style="color:var(--green)">Paid</div><div class="psl"><%= sw?"Ada 2025/26":"Fees 2025/26" %></div></div>
    <div class="pdsc"><div class="psi">📅</div><div class="psv">Y${student.yearOfStudy}/S${student.semester}</div><div class="psl"><%= sw?"Mwaka/Muhula":"Year/Sem" %></div></div>
    <div class="pdsc"><div class="psi">🎓</div><div class="psv">${student.courseName.length() > 12 ? student.courseName.substring(0,12)+'...' : student.courseName}</div><div class="psl"><%= sw?"Programu":"Program" %></div></div>
  </div>
  </c:if>
  ...
  <div style="background:#fff;border-radius:var(--radius);box-shadow:var(--shadow);overflow:hidden">
    <div class="ptbn">
      <button class="ptb active" onclick="showPortalTab('info',this)"><%= sw?"Taarifa":"Profile" %></button>
      <button class="ptb" onclick="showPortalTab('payments',this)"><%= sw?"Malipo":"Payments" %></button>
      <button class="ptb" onclick="showPortalTab('results',this)"><%= sw?"Matokeo":"Results" %></button>
      <button class="ptb" onclick="showPortalTab('docs',this)"><%= sw?"Nyaraka":"Documents" %></button>
    </div>
    <div style="padding:24px">
        ...
        <c:choose>
          ...
          <div class="grid2">
            <div style="background:var(--off);border-radius:8px;padding:18px">
              <h4 style="font-size:.9rem;color:var(--navy);margin-bottom:14px"><%= sw?"Taarifa za Mwanafunzi":"Student Information" %></h4>
              <div style="font-size:.83rem;color:var(--g700);display:flex;flex-direction:column;gap:8px">
                <div><strong><%= sw?"Jina Kamili:":"Full Name:" %></strong> ${student.fullName}</div>
                <div><strong><%= sw?"Nambari ya Usajili:":"Reg. No.:" %></strong> ${student.studentNumber}</div>
                <div><strong><%= sw?"Programu:":"Program:" %></strong> ${student.courseName}</div>
                <div><strong><%= sw?"Mwaka/Muhula:":"Year/Sem:" %></strong> Year ${student.yearOfStudy}, Semester ${student.semester}</div>
                <div><strong><%= sw?"Mwaka wa Udahili:":"Intake Year:" %></strong> ${student.intakeYear}</div>
                <div><strong><%= sw?"Simu:":"Phone:" %></strong> ${student.phone}</div>
                <div><strong><%= sw?"Barua Pepe:":"Email:" %></strong> ${student.email}</div>
              </div>
            </div>
            <div style="background:var(--off);border-radius:8px;padding:18px">
              <h4 style="font-size:.9rem;color:var(--navy);margin-bottom:14px"><%= sw?"Muhtasari wa Fedha":"Financial Summary" %></h4>
          ...
          <div style="text-align:center;padding:32px;color:var(--g400)">
            ...
            <p><%= sw?"Tafadhali ingia na nambari yako ya usajili.":"Please login with your student registration number." %></p>
          </div>
          </c:otherwise>
        </c:choose>
      </div>
      ...
      <div id="ptab-payments" class="ptbc">
        <c:choose>
          ...
          <table class="dt">
            <thead><tr><th><%= sw?"Nambari ya Udhibiti":"Control No." %></th><th><%= sw?"Maelezo":"Description" %></th><th><%= sw?"Kiasi":"Amount" %></th><th><%= sw?"Tarehe":"Date" %></th><th><%= sw?"Hali":"Status" %></th><th></th></tr></thead>
            <tbody>
              <c:forEach var="p" items="${payments}">
              <tr>
          ...
          <c:otherwise><p style="color:var(--g400);padding:20px;text-align:center"><%= sw?"Hakuna rekodi za malipo.":"No payment records found." %></p></c:otherwise>
        </c:choose>
      </div>
      ...
      <div id="ptab-results" class="ptbc">
        <table class="dt">
          <thead><tr><th><%= sw?"Kanuni":"Code" %></th><th><%= sw?"Somo":"Subject" %></th><th>CAT</th><th><%= sw?"Mtihani":"Exam" %></th><th><%= sw?"Jumla":"Total" %></th><th><%= sw?"Daraja":"Grade" %></th><th><%= sw?"Mahudhurio":"Attend." %></th></tr></thead>
          <tbody>
      ...
      <div id="ptab-docs" class="ptbc">
        <div style="display:flex;flex-direction:column;gap:8px">
          <a href="${pageContext.request.contextPath}/download?type=admission&ref=<c:out value='${student != null ? student.studentNumber : \"\"'}'/>" class="dltb">
            <span>📄 <%= sw?"Barua ya Udahili":"Admission Letter" %></span><span style="font-size:.75rem;color:var(--g400)">PDF ↓</span>
          </a>
          <c:forEach var="p" items="${payments}">
          <c:if test="${p.status=='PAID'}">
          <a href="${pageContext.request.contextPath}/download?type=receipt&ref=${p.controlNumber}" class="dltb">
            <span>🧾 <%= sw?"Stakabadhi ya Ada":"Fee Receipt" %> — ${p.controlNumber}</span><span style="font-size:.75rem;color:var(--g400)">PDF ↓</span>
          </a>
          </c:if>
          </c:forEach>
          <button class="dltb" onclick="alert('<%= sw?"Kadi itapatikana ofisini.":"ID card available at college office." %>')">
            <span>🪪 <%= sw?"Kadi ya Kitambulisho":"Student ID Card" %></span><span style="font-size:.75rem;color:var(--g400)">PDF ↓</span>
          </button>
          <button class="dltb" onclick="alert('<%= sw?"Muhtasari unapatikana baada ya mtihani.":"Transcript available after exams." %>')">
            <span>📊 <%= sw?"Muhtasari wa Masomo":"Academic Transcript" %></span><span style="font-size:.75rem;color:var(--g400)">PDF ↓</span>
          </button>
        </div>
      </div>
    </div>
